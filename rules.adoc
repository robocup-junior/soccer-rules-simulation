= RoboCupJunior Soccer Simulation Rules 2021
{docdate}
:toc: left
:sectanchors:
:sectlinks:
...
These are the official Soccer Simulation rules for RoboCupJunior 2021. They are released
...
[discrete]
=== Changes in comparison with the 2020 RoboCupJunior Soccer Rules
...
{++No robots are supposed to be inside any penalty area for more than 10 seconds.
After this time, they will be placed on the nearest unoccupied neutral spot.
For this rule to apply, a robot must be inside the penalty area with its center
of mass.++}
...
{++Sometimes robots in the simulation will tip or fall over. The artificial referee
will re-spawn any robots that do not move for 15 seconds onto the neirest unoccupied
neutral spot.++}
...
Each team is allowed to have at most {~~two~>three~~} robots for the full tournament.
...
{++There is no rule about communication.++}
...
well as drive forward and backward anywhere on the field. There is no restriction
on agility.++}

[[textures]]
=== Textures

{++Robots can have customized textures.
These textures cannot be transparent or carpet-green.++}
...
image:media/soccer_sim_field.png[image,scaledwidth=70.0%]

image:media/soccer_sim_field_topview.png[image,scaledwidth=70.0%]
...
Robots are not allowed to cause interference with other
robots or the referee during normal game play.

Robots are not allowed to cause interference to the field or to the ball during
normal game play.
